...
const V68_Patch v68_patches[12] = {
    ...
    [V68_PATCH_STRINGS] = {
        .op = {
            { 0x61, 0x23, 0x10, 0x06, 0x00, 0x16, 0x00 },
            { 0x41, 0x08, 0x0F, 0x04, 0x00, 0x16, 0x00 },
            { 0x31, 0x06, 0x0E, 0x04, 0x00, 0x16, 0x00 },
            { 0x51, 0x06, 0x12, 0x04, 0x00, 0x16, 0x00 },
        },
        .fb_alg = 0x0E,
        .echo = 1,
    },
    ...
    [V68_PATCH_HARP] = {
        .op = {
            { 0x41, 0x0A, 0x1F, 0x0A, 0x08, 0x6A, 0x00 },
            { 0x42, 0x14, 0x1F, 0x09, 0x06, 0x59, 0x00 },
            { 0x43, 0x1A, 0x1F, 0x08, 0x05, 0x59, 0x00 },
            { 0x41, 0x10, 0x1F, 0x08, 0x06, 0x8A, 0x00 },
        },
        .fb_alg = 0x11,
        .echo = 0,
    },
...
void v68_sound_tick(void) {
    ...
    for (u8 t = 0; t < section_tracks; t++) {
        ...
        if (st->frames_left > 0) {
            st->frames_left--;
            // release a tick early so the next key-on lands on a decayed tail
            if (st->frames_left == 0 && !dispatched && trk->ch < 8)
                ch_silence(trk->ch);
        }
    }
